// third_party_effects_data_gen — 基于 Excel 配置表生成游戏特效/效果数据的命令行工具
// 从 .xlsx 读取参数配置，输出 C 头文件和二进制效果数据包
#include <iostream>
...
FzParseResult parse_frame_size_type(const std::string &input_str, int max_gears = 16)
{
    ...
    std::regex gear_pattern(R"(^\{\s*\d+\s*(,\s*\d+\s*)*\}$)");
    std::smatch match_results;
    if (std::regex_match(trimmed, match_results, gear_pattern))
    {
        // 提取数字
        std::regex number_pattern(R"(\d+)");
        std::sregex_iterator it(trimmed.begin(), trimmed.end(), number_pattern);
    ...
    // 类型3：[1:256]，支持任意空�?
    std::regex range_pattern(R"(^\[\s*\d+\s*:\s*\d+\s*\]$)");
    std::smatch range_match;
    if (std::regex_match(trimmed, range_match, range_pattern))
    {
        // 提取数字
        std::regex number_pattern(R"(\d+)");
        std::sregex_iterator it(trimmed.begin(), trimmed.end(), number_pattern);
...
{
    int bytesNum = 0;
    std::regex reHexAByte("(0x[0-9a-fA-F][0-9a-fA-F])");

    auto begin = std::sregex_iterator(str.begin(), str.end(), reHexAByte);
...
std::string update_file_checksum_name(const fs::path &filePath, uint32_t newChecksum, bool has_3rd)
{
    ...
    std::regex expr(R"(^(roboeffect_library_info)(?:_(v\d+\.\d+\.\d+))?)");
    std::smatch what;
    std::string prefix_name, version_num;

    if (std::regex_search(filename, what, expr)) {
...
int main(int argc, char **argv)
{
    ...
    try
    {
        // Manual CLI parsing (replaces boost::program_options)
        for (int i = 1; i < argc; ++i) {
            std::string arg(argv[i]);
            if (arg == "-h" || arg == "--help") {
                std::cout << "Options:" << std::endl;
                std::cout << "  -h [ --help ]         show help informations" << std::endl;
                std::cout << "  -p [ --process ] arg  processing folder" << std::endl;
                std::cout << "  -c [ --check ] arg    check bin file" << std::endl;
                return 0;
                ...
                if (i + 1 < argc) {
                    check_bin_full_path = argv[++i];
                }
            }
        }

        if (!check_bin_full_path.empty())
        {
            try
            {
    ...
    {
        if (entry.is_regular_file() && entry.path().filename().string().rfind("roboeffect_library_info", 0) == 0)
        {
    ...
    for (const auto &entry : fs::directory_iterator(work_folder))
    {
        ...
        std::regex temp_regext("roboeffect_library_info_(.*?)_v\\d+\\.\\d+\\.\\d+\\.bin");
        std::smatch result;

        // if (std::regex_match(filename, result, temp_regext)) {

        // 	auto fileSize = fs::file_size(entry.path());
        // 	fmt::print("{} is a 3rd party info bin file, size={}\n", filename, fileSize);
        // }

        if (entry.is_regular_file() && ends_with(filename, "_3rdparty.bin"))
        {
